...
export async function GET(request: NextRequest) {
  try {
    ...
    if (since) {
      sessionsQuery = sessionsQuery.gt('client_updated_at', since);
    }
    ...
    if (sessions) {
      for (const session of sessions) {
        changes.push({
          ...
          updated_at: session.client_updated_at || session.created_at,
        });
      }
    }

    // Fetch workout sets - skip if no sessions
    ...
    if (sessionIds.length > 0) {
      ...
      if (since) {
        setsQuery = setsQuery.gt('client_updated_at', since);
      }
      ...
      if (sets) {
        for (const set of sets) {
          ...
          const sessionClientId = session?.client_id || set.session_id;
          ...
          changes.push({
            ...
            updated_at: set.client_updated_at || set.completed_at,
          });
        }
      }
    }
